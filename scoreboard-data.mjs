export const cacheTtlMs = Number(process.env.SCOREBOARD_CACHE_MS || 8000);
...
function parseGame(tokens, start) {
  ...
  return {
    ...
    live: {
      away: Number(tokens[start + 1]),
      home: Number(tokens[start + 3]),
...
function reverseCompletedGames(standings, games) {
  ...
  return rankStandings(
    Array.from(byId.values()).map((team) => ({
      ...team,
...
export async function scoreboardResponse() {
